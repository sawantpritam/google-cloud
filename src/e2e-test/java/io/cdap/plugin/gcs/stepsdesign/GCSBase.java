...
public class GCSBase implements E2EHelper {
  ...
  public void verifyDataIsTransferredToTargetGCSBucketWithFileNamePrefix(String fileNamePrefix) {
    ...
    if (verifyFilePresentInGcsBucket(gcsBucket, fileNamePrefix, "prefix")) {
  ...
  private boolean verifyFilePresentInGcsBucket(String gcsBucket, String filename, String filePart) {
    try {
      for (Blob blob : StorageClient.listObjects(gcsBucket).iterateAll()) {
        ...
        if (filePart.equalsIgnoreCase("prefix")) {
          if (blobFileName.startsWith(filename)) {
            return true;
          }
        }
        if (filePart.equalsIgnoreCase("suffix")) {
          if (blobFileName.endsWith(filename)) {
            return true;
          }
        }
        if (filePart.equalsIgnoreCase("contains")) {
          if (blobFileName.contains(filename)) {
            return true;
          }
        }
        if (filePart.equalsIgnoreCase("equals")) {
